...
import { deleteUserPrompt, getCreatorPrompts, updatePrompt } from "@/lib/api/prompts";
...
export default function CreatorMyPromptsHomePage() {
  ...
  const handleAction = async (actionType, promptId) => {
    if (actionType === "view") {
      ...
      router.push(`/dashboard/creator/analytics/${promptId}`);
    }
  };
              ...
              <button
                onClick={async () => {
                  ...
                  if (result) {
                    ...
                    setPrompts(prev => prev.map(p => p._id === editPrompt._id ? editPrompt : p));
                    setEditPrompt(null);
                  } else {
