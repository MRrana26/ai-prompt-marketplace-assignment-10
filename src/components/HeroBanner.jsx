...
const getRandomTags = () => [...AllTags].sort(() => 0.5 - Math.random()).slice(0, 5);

export default function HeroBanner() {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = useState("");
  const [trendingTags] = useState(getRandomTags);
  ...
  return (
    <div className="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-[#09090b] px-4 sm:px-6 lg:px-8 border-b border-zinc-900 pt-3">
      ...
      >
        ...
        >
          ...
          {trendingTags.map((tag, index) => (
            <Button
              key={index}
              onClick={() => router.push(`/all-prompts?search=${encodeURIComponent(tag)}`)}
        ...
        >
          <Link href={'all-prompts'}>
            <Button className="bg-linear-to-r from-purple-600 via-pink-600 to-amber-500 hover:from-purple-500 hover:via-pink-500 hover:to-amber-400 text-white font-semibold px-7 py-6 rounded-xl transition-all active:scale-95 shadow-lg shadow-purple-500/20 text-sm sm:text-base tracking-wide">
              Browse Marketplace
            </Button>
          </Link>
        </motion.div>
