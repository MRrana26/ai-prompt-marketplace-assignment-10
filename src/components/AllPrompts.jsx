...
const AllPromptsHomePage = ({ searchQuery = '' }) => {
    ...
    useEffect(() => {
        const fetchPromptsData = async () => {
            try {
                setLoading(true);
                const data = await getPublicPrompts(currentPage, LIMIT);
    ...
    const filteredPrompts = useMemo(() => {
        ...
        return updatedList;
    }, [localSearch, selectedEngine, selectedCategory, selectedDifficulty, sortBy, prompts]);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                    ...
                    <div className="bg-[#0b132b]/40 border border-zinc-900 rounded-2xl p-5 sticky top-6 backdrop-blur-sm">
                        <div className="flex items-center justify-between mb-6 border-b border-zinc-800/60 pb-3">
                            ...
                            <button onClick={handleResetFilters} className="text-xs text-zinc-500 hover:text-purple-400 transition-colors">
                                Reset all
                            </button>
                        </div>
                            ...
                            <div className="flex flex-col gap-1">
                                {['All', 'ChatGPT', 'Gemini', 'Claude', 'Midjourney', 'Stable Diffusion', 'Other'].map((engine) => (
                                    <button
                                        ...
                                        className={`text-left text-xs px-3 py-2 rounded-lg transition-all ${selectedEngine === engine ? 'bg-purple-950/40 text-purple-400 font-semibold border border-purple-900/50' : 'text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-200'}`}
                                    >
                                        {engine}
                                    </button>
                                ))}
                            ...
                            <div className="flex flex-col gap-1">
                                {['All', 'Coding', 'Writing', 'Marketing', 'Graphics & Image', 'Idea Generation', 'System Assistant', 'Other'].map((cat) => (
                                    <button
                                        ...
                                        className={`text-left text-xs px-3 py-2 rounded-lg transition-all ${selectedCategory === cat ? 'bg-purple-950/40 text-purple-400 font-semibold border border-purple-900/50' : 'text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-200'}`}
                                    >
                                        {cat}
                                    </button>
                                ))}
                            ...
                            <div className="flex flex-col gap-1">
                                {['All', 'Beginner', 'Intermediate', 'Pro'].map((diff) => (
                                    <button
                                        ...
                                        className={`text-left text-xs px-3 py-2 rounded-lg transition-all ${selectedDifficulty === diff ? 'bg-purple-950/40 text-purple-400 font-semibold border border-purple-900/50' : 'text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-200'}`}
                                    >
                                        {diff}
                                    </button>
                                ))}
                            ...
                            <div className="flex gap-1.5">
                                {['Latest', 'Most Popular', 'Most Copied'].map((tab) => (
                                    <button
                                        ...
                                        className={`text-xs px-4 py-1.5 rounded-lg font-medium transition-all ${sortBy === tab ? 'bg-zinc-800 text-white border border-zinc-700/60' : 'text-zinc-400 hover:text-zinc-200'}`}
                                    >
                                        {tab}
                                    </button>
                                ))}
                                    ...
                                    {filteredPrompts.map((prompt) => (
                                        ...
                                        >
                                            <div>
                                                ...
                                                <div className="relative overflow-hidden rounded-2xl border border-default-200 shadow-md hover:shadow-xl transition-all duration-300 group mb-3">
                                                    <Image
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-10">
                        <button
                            ...
                            className="px-4 py-2 rounded-xl text-xs font-semibold bg-zinc-900 border border-zinc-800 text-zinc-300 hover:border-purple-500/50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                        >
                            Previous
                        </button>
                            ...
                            <button
                                ...
                                className={`w-9 h-9 rounded-xl text-xs font-bold transition-all ${
                                    currentPage === page
                                        ...
                                        : "bg-zinc-900 border border-zinc-800 text-zinc-400 hover:border-purple-500/50"
                                }`}
                            >
                                {page}
                            </button>
